...
const GraphViewer: React.FC<GraphViewerProps> = ({
  ...
  const handleBuildIndex = async () => {
    try {
      setIndexing(true);
      await agent.Index.build();
    } catch (error) {
      console.error("An error occurred while building the index.", error);
    } finally {
      setIndexing(false);
    }
  };
    ...
    >
      ...
      >
        ...
        <Button
          ...
          disabled={!serverUp || indexing}
        >
          {indexing ? <CircularProgress size={18} /> : "Rebuild Index"}
        </Button>
        <Button
          variant="contained"
